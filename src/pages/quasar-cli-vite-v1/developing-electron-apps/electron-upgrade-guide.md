...
desc: (legacy @quasar/app-vite v1) Upgrading instructions when dealing with Electron in Quasar.
scope:
  oldStructure:
    l: src-electron
    c:
...
More info: [preload script](/quasar-cli-vite-v1/developing-electron-apps/electron-preload-script).
...
### Renderer thread (/src)
The [$q object](https://v2.quasar.dev/options/the-q-object) no longer contains the `electron` property. You will need to use the [preload script](/quasar-cli-vite-v1/developing-electron-apps/electron-preload-script) to access it and provide it to the renderer thread.
