---
title: Ajax Requests
desc: (legacy @quasar/app-vite v1) Using Axios for fetching data in a Quasar app.
---
...
::: tip
Be sure to check out [Prefetch Feature](/quasar-cli-vite-v1/prefetch-feature) if you are using Quasar CLI.
:::

Usage in your single file components methods will be like below. Notice that in the next example we're using the Quasar's [Notify plugin](https://v2.quasar.dev/quasar-plugins/notify) (through `$q = useQuasar()` and `$q.notify`) which you'll need to install (follow the link earlier).
