...
desc: (legacy @quasar/app-vite v1) How to manage the Vuex Store in a Quasar App.
scope:
  tree:
    l: src
    c:
...
::: danger
The Vue Team deprecated Vuex in favor of [Pinia](/quasar-cli-vite-v1/state-management-with-pinia).
:::
...
::: tip
If you are developing a SSR app, then you can check out the [ssrContext](/quasar-cli-vite-v1/developing-ssr/ssr-context) Object that gets supplied server-side.
:::
...
Similarly, you can also use a typed store when using the [Prefetch feature](/quasar-cli-vite-v1/prefetch-feature). Here is an example:
...
You can take advantage of the [PreFetch Feature](/quasar-cli-vite-v1/prefetch-feature#store-code-splitting) to code-split Vuex modules.
...
The first method is using the [PreFetch Feature](/quasar-cli-vite-v1/prefetch-feature#store-code-splitting) that Quasar offers, similar to the example for regular Vuex, found [here](/quasar-cli-vite-v1/prefetch-feature#store-code-splitting). To do this, we have a route defined in our `router/routes.ts` file. For this example, we have a /admin route which is a child of our MainLayout:
...
To do this, you can use a [Boot File](/quasar-cli-vite-v1/boot-files) in Quasar that looks like this:
