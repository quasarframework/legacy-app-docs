...
desc: (legacy @quasar/app-webpack v4) The things you should know about security in a Quasar desktop app.
---
...
1.  Make sure that you leave `webPreferences` > `contextIsolation` set to `true`. Use the [preload script](/quasar-cli-webpack-v4/developing-electron-apps/electron-preload-script) to inject only must-have APIs to the renderer thread.
