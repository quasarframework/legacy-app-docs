...
desc: (legacy @quasar/app-webpack v3) How to manage the Vuex Store in a Quasar App.
scope:
  tree:
    l: src
    c:
...
::: danger
The Vue Team deprecated Vuex in favor of [Pinia](/quasar-cli-webpack-v3/state-management-with-pinia).
:::
...
Similarly, you can also use a typed store when using the [Prefetch feature](/quasar-cli-webpack-v3/prefetch-feature). Here is an example:
...
You can take advantage of the [PreFetch Feature](/quasar-cli-webpack-v3/prefetch-feature#store-code-splitting) to code-split Vuex modules.
...
The first method is using the [PreFetch Feature](/quasar-cli-webpack-v3/prefetch-feature#store-code-splitting) that Quasar offers, similar to the example for regular Vuex, found [here](/quasar-cli-webpack-v3/prefetch-feature#store-code-splitting). To do this, we have a route defined in our `router/routes.ts` file. For this example, we have a /admin route which is a child of our MainLayout:
...
To do this, you can use a [Boot File](/quasar-cli-webpack-v3/boot-files) in Quasar that looks like this:
