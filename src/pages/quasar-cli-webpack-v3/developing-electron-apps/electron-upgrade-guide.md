...
desc: (legacy @quasar/app-webpack v3) Upgrading instructions when dealing with Electron in Quasar.
scope:
  oldStructure:
    l: src-electron
    c:
...
More info: [preload script](/quasar-cli-webpack-v3/developing-electron-apps/electron-preload-script).
...
### Renderer thread (/src)
The [$q object](https://v2.quasar.dev/options/the-q-object) no longer contains the `electron` property. You will need to use the [preload script](/quasar-cli-webpack-v3/developing-electron-apps/electron-preload-script) to access it and provide it to the renderer thread.

Furthermore, the [openURL](https://v2.quasar.dev/quasar-utils/other-utils#open-external-url) util can no longer tap into Electron to open a new window. You will need to provide your own util from the preload script.
