...
desc: (legacy @quasar/app-webpack v3) How to manage your server-side rendered apps with Quasar CLI.
related:
...
> If you decide to go with a PWA client takeover (**which is a killer combo**), the Quasar CLI PWA mode will be installed too. You may want to check out the [Quasar PWA](/quasar-cli-webpack-v3/developing-pwa/introduction) guide too. But most importantly, make sure you read [SSR with PWA](/quasar-cli-webpack-v3/developing-ssr/ssr-with-pwa) page.
...
If you want more information, please see this page that goes into more detail about [handling webpack](/quasar-cli-webpack-v3/handling-webpack) in the `/quasar.config` file.
...
However, should you wish to manually hydrate it yourself, you need to set quasar.config file > ssr > manualStoreHydration: true. One good example is doing it from [a boot file](/quasar-cli-webpack-v3/boot-files):
...
4. The `/src-ssr/production-export.js` file is detailed in [SSR Production Export](/quasar-cli-webpack-v3/developing-ssr/ssr-production-export) page. Read it especially if you need to support serverless functions.
...
When running on SSR mode, your application code needs to be isomorphic or "universal", which means that it must run both on a Node context and in the browser. This applies to your [Boot Files](/quasar-cli-webpack-v3/boot-files) too.
...
When a boot file runs on the server, you will have access to one more parameter (called [ssrContext](/quasar-cli-webpack-v3/developing-ssr/ssr-context)) on the default exported function:
