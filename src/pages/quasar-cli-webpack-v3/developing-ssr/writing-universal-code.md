...
desc: (legacy @quasar/app-webpack v3) Guide on how to write code for a Quasar server-side rendered app.
---
...
When you add a 3rd party library to your project (through a [Boot File](/quasar-cli-webpack-v3/boot-files)), take into consideration whether it can run on server and on client. If it needs to run only on server or only on client, then specify this in the `/quasar.config` file:
...
The Quasar CLI [PreFetch Feature](/quasar-cli-webpack-v3/prefetch-feature) has been created to solve this problem. Take a few moments to read about it.
